...
const char* eRazredbA[] = { "","PRVI","DRUGI","TRECI","CETVRTI" };
...
public:
	...
	Uspjeh(Uspjeh &u):_razred(u._razred) {
	...
	friend ostream& operator<< (ostream &COUT, Uspjeh &obj) {
		COUT <<eRazredbA[obj._razred] << " RAZRED: " << crt;
...
public:
	...
	bool AddPredmet(eRazred r, Predmet p, DatumVrijeme d) {
		//fali po datumu provjera zato ne radi dobro//
		for (vector<Uspjeh*>::iterator i = _uspjeh.begin(); i != _uspjeh.end(); i++)
		{
			...
			if (u->GetRazred() == r) {
				...
				{
					if (*u->GetPredmeti()->getElement1(j) == p) return false;
				}
				u->GetPredmeti()->AddElement(new Predmet(p), d);
				if (u->GetPredmeti()->getTrenutno() == 6) {
					string sadrzaj = "evidentirali ste uspjeh za " + string(eRazredbA[u->GetRazred()]) + " razred";
					thread nit1(ZaMail, sadrzaj);
					nit1.join();

					float ukupnoOcjene = 0;
					int brOcjena = 0;
					for (int i = 0; i < u->GetPredmeti()->getTrenutno(); i++) {
						ukupnoOcjene += u->GetPredmeti()->getElement1(i)->GetOcjena();
						brOcjena++;
					}
					float prosjek = ukupnoOcjene / brOcjena;
					if (prosjek > minimalan_prosjek) {
						string prosjekk = "svaka cast za uspjeh " + to_string(prosjek);
						thread nit2(Sms,prosjekk );
						nit2.join();
					}
		...
		Uspjeh *noviUspjeh = new Uspjeh(r);
		noviUspjeh->DodajPredmet(p, d);
